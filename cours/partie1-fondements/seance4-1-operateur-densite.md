# Séance 4.1 — Opérateur densité et systèmes composites
...
où $p_i \geq 0$, $\sum_i p_i = 1$.

**Propriétés :**
- $\rho^\dagger = \rho$ (hermitienne)
- $\rho \geq 0$ (positive semi-définie)
- $\text{Tr}(\rho) = 1$
- $\text{Tr}(\rho^2) \leq 1$ (égalité ssi état pur)
...
État pur : $\ket{+} = \frac{1}{\sqrt{2}}(\ket{0} + \ket{1})$
